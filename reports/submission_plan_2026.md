...
- The tuned-fusion frontier now has ten target-transform checks. The same source-selected cap_0p4 policy is stable under JPEG70 at 0.7661 accuracy / 0.8485 AUC, JPEG50 at 0.7515 / 0.8309, noise3 at 0.7690 / 0.8704, social-square crop/recompression at 0.7778 / 0.8474, and long-edge social-720p downscale/recompression at 0.7602 / 0.8506; it is weaker under JPEG30 at 0.7076 / 0.8167, blur1 at 0.7105 / 0.7872, resize_half at 0.7164 / 0.7816, crop85 at 0.7251 / 0.8227, and screenshot-style roundtrip at 0.7310 / 0.7965. This is now a real robustness stress result rather than a single JPEG-only check.
- A native-resolution tiling diagnostic for the reverse `combined_v3` conventional branch is useful but not a free win: global resized scoring reaches 0.5468 accuracy / 0.5772 AUC, while top-2 tile averaging keeps accuracy at 0.5468 and improves AUC to 0.5936 at the cost of a 0.8275 fake-call rate. Use it as calibration/failure-analysis evidence, not as a new SCP-Fusion headline result.
- Feeding native-tiled `combined_v3` scores into the fixed reverse tuned-fusion stack gives a small but real fused diagnostic gain: `tile_mean` reaches 0.7749 accuracy and `tile_max` reaches 0.8472 AUC, versus the clean global cap_0p4 result at 0.7632 / 0.8361. Neural and foundation branches remain global-image scores, so this is bounded evidence for local physical/signal complementarity rather than a full tiled-branch retraining result.
...
1. Improve logit-level fusion:
   ...
   - utility-aware binary source-threshold selection, model-side selection, source-heldout generator selection, constrained source-heldout fusion tuning, source fake-rate constraint sweeps, ten target-transform checks, and a fused native-tiling diagnostic are now implemented; the opposite-direction threshold-objective sweep shows the 0.40 cap frontier is directional, so the next step is a larger source split or true tiled inference for neural/foundation branches.
