...
- The tuned-fusion frontier now has seven target-transform checks. The same source-selected cap_0p4 policy is stable under JPEG70 at 0.7661 accuracy / 0.8485 AUC, JPEG50 at 0.7515 / 0.8309, and noise3 at 0.7690 / 0.8704; it is weaker under blur1 at 0.7105 / 0.7872, resize_half at 0.7164 / 0.7816, crop85 at 0.7251 / 0.8227, and screenshot-style roundtrip at 0.7310 / 0.7965. This is now a real robustness stress result rather than a single JPEG-only check.
...
1. Improve logit-level fusion:
   ...
   - utility-aware binary source-threshold selection, model-side selection, source-heldout generator selection, constrained source-heldout fusion tuning, source fake-rate constraint sweeps, and four target-transform checks are now implemented; the opposite-direction threshold-objective sweep shows the 0.40 cap frontier is directional, so the next step is harder perturbations such as JPEG50, screenshot/noise, native-resolution tiling, or a larger source split.
