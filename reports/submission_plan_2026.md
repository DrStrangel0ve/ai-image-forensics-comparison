...
- A first robustness check for that tuned-fusion frontier is positive: under JPEG70 target recompression, the same source-selected cap_0p4 policy reaches 0.7661 accuracy / 0.8485 AUC with a 0.4678 target fake-call rate.
...
1. Improve logit-level fusion:
   ...
   - utility-aware binary source-threshold selection, model-side selection, source-heldout generator selection, constrained source-heldout fusion tuning, source fake-rate constraint sweeps, and a first JPEG70 robustness check are now implemented; the opposite-direction threshold-objective sweep shows the 0.40 cap frontier is directional, so the next step is blur/resize/crop robustness or a larger source split.
