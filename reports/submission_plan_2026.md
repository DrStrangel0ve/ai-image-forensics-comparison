...
1. Improve logit-level fusion:
   ...
   - utility-aware binary source-threshold selection, model-side selection, source-heldout generator selection, constrained source-heldout fusion tuning, source fake-rate constraint sweeps, and ten target-transform checks are now implemented; the opposite-direction threshold-objective sweep shows the 0.40 cap frontier is directional, and the branch-level native-tiling diagnostic shows local crops can shift ranking while worsening fake-call bias, so the next step is full fused-model tiling or a larger source split.
