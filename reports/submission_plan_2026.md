...
1. Improve logit-level fusion:
   ...
   - utility-aware binary source-threshold selection, model-side selection, and source-heldout generator selection are now implemented and tested; all point to the same conclusion that source fake-rate constraints help, while utility alone overfits source behavior, so the next step is constrained held-out-generator utility training rather than more selection-only tuning.
