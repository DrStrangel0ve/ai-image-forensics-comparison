...
The zero-shot transfer result points in the opposite direction from the Ishu in-dataset result: training-domain coverage and score calibration still matter more than raw same-dataset accuracy. Across seeds 7, 17, and 29, Ishu-to-MS COCOAI fusion has the best mean transfer score at 0.6060 accuracy / 0.6637 AUC, ahead of ResNet-18 at 0.5800 / 0.6488, but seed 17 remains a clear ResNet win. The seed-17 source-label diagnosis shows fusion is too conservative across most generated MS COCOAI sources, especially Midjourney 6, SD3, SD2.1, and SDXL. A stronger-regularization fusion probe improved seed-17 default transfer accuracy from 0.5400 to 0.5670 but reduced AUC from 0.6306 to 0.6152, so it did not solve the score-separation failure. MS COCOAI back to Ishu found fusion ahead by AUC, and source-domain threshold calibration makes it the best transfer-accuracy model too. The next best robustness step is to test source-aware validation or transfer-style augmentation.
