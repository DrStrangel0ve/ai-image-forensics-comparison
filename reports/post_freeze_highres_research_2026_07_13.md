...
The first full probe is configured as a private Kaggle script at `kaggle/freuid_post_freeze_highres_research`. It requests the `NvidiaTeslaT4` accelerator and never creates a competition submission. The runner first looks for an attached competition input; if Kaggle does not preserve that metadata, it calls the officially supported `kagglehub.competition_download` API with Kaggle's metadata cache disabled. This forces a server-side HTTP download into `/kaggle/temp/freuid-competition`, avoiding the non-interactive attachment restriction while keeping the 17.58 GB dataset off the desktop and out of persistent notebook output. The run caches normalized DINOv2-B embeddings once, then compares fixed logistic probes over mean, mean-plus-max, and mean-plus-max-plus-standard-deviation crop statistics.
