...
The first full probe is configured as a private Kaggle script at `kaggle/freuid_post_freeze_highres_research`. It requests the `NvidiaTeslaT4` accelerator and never creates a competition submission. The runner first looks for an attached competition input; if Kaggle does not preserve that metadata, it calls the officially supported `kagglehub.competition_download` API. Inside a Kaggle notebook, that API authenticates automatically and serves the competition resource from Kaggle's shared cache, so the 17.58 GB dataset is not downloaded to the desktop. The run caches normalized DINOv2-B embeddings once, then compares fixed logistic probes over mean, mean-plus-max, and mean-plus-max-plus-standard-deviation crop statistics.
