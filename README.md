...
A checked-in smoke test for this path is generated by [scripts/build_competition_submission_dry_run.py](scripts/build_competition_submission_dry_run.py) and summarized in [reports/competition_submission_dry_run_2026_06_15.md](reports/competition_submission_dry_run_2026_06_15.md). It converts an existing per-image score table into a small upload-shaped CSV and lints it against expected IDs.

The FREUID Challenge 2026 Kaggle track uses an exact `id,label` submission format and low-BPCER document-fraud metrics. Its setup, current Kaggle status, and first-run plan are documented in [reports/freuid_challenge_2026_setup.md](reports/freuid_challenge_2026_setup.md), with release-state recovery notes in [reports/freuid_release_runbook.md](reports/freuid_release_runbook.md). Use [scripts/download_freuid_images.py](scripts/download_freuid_images.py) for Kaggle's nested image paths, [scripts/materialize_freuid_local_images.py](scripts/materialize_freuid_local_images.py) to convert whatever train/public-test images are already on disk into clean metadata CSVs, [scripts/prepare_freuid_split.py](scripts/prepare_freuid_split.py) to create a type/label-stratified local validation split, [scripts/run_freuid_feature_baseline.py](scripts/run_freuid_feature_baseline.py) for conventional/image-feature scoring, [scripts/run_freuid_frozen_encoder_baseline.py](scripts/run_freuid_frozen_encoder_baseline.py) for frozen encoder validation and public-test scoring via `--test-csv`, [scripts/check_freuid_release_state.py](scripts/check_freuid_release_state.py) to verify Kaggle source attachment and candidate readiness, [scripts/package_freuid_submission.py](scripts/package_freuid_submission.py) to package predictions, and [scripts/lint_freuid_submission.py](scripts/lint_freuid_submission.py) before any upload.
...
A three-seed tiled CLIP reverse-transfer pass is checked into [reports/tiled_clip_reverse_transfer_2026_06_14.md](reports/tiled_clip_reverse_transfer_2026_06_14.md). On MS-COCOAI-to-Ishu, tile-mean scoring improves default accuracy/Brier/ECE from 0.6228 / 0.3317 / 0.3566 to 0.6784 / 0.2638 / 0.2893, while top-2 tile scoring lifts AUC from 0.8244 to 0.8935 but remains fake-call heavy.
A cross-encoder tiled foundation comparison is checked into [reports/tiled_foundation_reverse_transfer_comparison_2026_06_14.md](reports/tiled_foundation_reverse_transfer_comparison_2026_06_14.md). With the same MS-COCOAI-to-Ishu protocol, ConvNeXt-Tiny plus `tile_mean` is the best forced-decision tiled encoder at 0.7310 accuracy, while CLIP plus `tile_top2_mean` is the best ranker at 0.8935 AUC. DINOv2-small also benefits from tiling, reaching 0.6550 accuracy with `tile_mean` and 0.7558 AUC with `tile_top2_mean`, but it remains behind the other foundation branches in this reverse direction.
A fused tiled-foundation comparison is checked into [reports/ms_cocoai_to_ishu_tiled_foundation_fusion_comparison_2026_06_14.md](reports/ms_cocoai_to_ishu_tiled_foundation_fusion_comparison_2026_06_14.md). Replacing a single target branch inside the selected reverse SCP-Fusion stack shows a small new fused frontier: DINOv2 `tile_top2_mean` reaches 0.7778 accuracy / 0.8490 AUC, edging past the prior native-tiled `combined_v3` branch at 0.7749 / 0.8472. ConvNeXt `tile_mean` has the highest AUC at 0.8492 but lower accuracy, so this remains a diagnostic gain rather than a SOTA claim.
A resize-half tiled-foundation stress probe is checked into [reports/ms_cocoai_to_ishu_tuned_fusion_resize_half_tiled_dinov2_2026_06_14.md](reports/ms_cocoai_to_ishu_tuned_fusion_resize_half_tiled_dinov2_2026_06_14.md). Standalone DINOv2 tiling is weak under half-resolution resize, topping out at 0.6374 accuracy with `tile_mean` and 0.7412 AUC with `tile_top2_mean`, but replacing the DINOv2 branch inside the fixed reverse SCP-Fusion stack improves the existing resize-half result from 0.7164 / 0.7816 to 0.7310 / 0.7956 accuracy/AUC with `tile_max`. Brier/ECE worsen slightly, so this is evidence for robust ranking/decision support, not better calibration.
A blur/JPEG/resize/screenshot tiled-DINO stress comparison is checked into [reports/tiled_dinov2_transform_stress_comparison_2026_06_14.md](reports/tiled_dinov2_transform_stress_comparison_2026_06_14.md). Blur1 improves from 0.7105 / 0.7872 to 0.7251 / 0.8032 accuracy/AUC with fused DINOv2 `tile_max`, JPEG30 improves from 0.7076 / 0.8167 to 0.7222 / 0.8292, resize-half improves from 0.7164 / 0.7816 to 0.7310 / 0.7956, and screenshot-style processing improves from 0.7310 / 0.7965 to 0.7427 / 0.8128. The repeated pattern is useful: standalone tiled DINO is not strong under these transforms, but as a fused branch it gives a small robust decision/ranking lift with mixed calibration.
A tiled-DINO calibration tradeoff report is checked into [reports/tiled_dinov2_calibration_tradeoff_2026_06_14.md](reports/tiled_dinov2_calibration_tradeoff_2026_06_14.md). Across the four checked transforms, `tile_max` is the best decision/ranking mode with average deltas of +0.0139 accuracy and +0.0147 AUC, while `tile_mean` is the safer calibration mode, improving Brier on 4/4 transforms and ECE on 3/4. This gives the paper a clean caveat: use `tile_max` for robustness operating-point gains and `tile_mean` for calibration-aware analysis.
