...
The FREUID Challenge 2026 Kaggle track uses an exact `id,label` submission format and low-BPCER document-fraud metrics. Its setup, current Kaggle status, and first-run plan are documented in [reports/freuid_challenge_2026_setup.md](reports/freuid_challenge_2026_setup.md). Use [scripts/prepare_freuid_split.py](scripts/prepare_freuid_split.py) to create a type/label-stratified local validation split, [scripts/package_freuid_submission.py](scripts/package_freuid_submission.py) to package predictions, and [scripts/lint_freuid_submission.py](scripts/lint_freuid_submission.py) before any upload.
