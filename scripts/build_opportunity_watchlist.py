...
def build_watchlist() -> tuple[str, pd.DataFrame]:
    frame = pd.DataFrame(OPPORTUNITIES)
    frame.insert(4, "days_until_deadline", frame["deadline"].map(_days_until))
    frame["urgency"] = frame.apply(_urgency, axis=1)
    return _write_markdown(frame), frame
