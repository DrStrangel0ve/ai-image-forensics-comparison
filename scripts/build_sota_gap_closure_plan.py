...
TASKS = [
    ...
    },
    {
        ...
        "status": "clip_reverse_pass_complete",
        ...
        "primary_gap": "CLIP reverse-transfer tiling is now benchmarked; DINOv2/ConvNeXt tiling and source-heldout fused use are still pending.",
        "commands": "Run `python scripts/evaluate_tiled_frozen_encoder_model.py --model-template <frozen_run_template> --target-template <target_template> --asset-prefix <prefix> --output-dir <out>` for DINOv2/ConvNeXt next, then compare max/mean/top-k tile pooling against current global scores.",
        "exit_criterion": "Fold the strongest tiled foundation operating point into source-heldout SCP-Fusion and test whether it improves AUC beyond 0.8472 or reduces resize/blur stress losses.",
