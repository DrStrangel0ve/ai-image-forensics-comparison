...
TASKS = [
    ...
    },
    {
        ...
        "status": "standalone_proxy_ready_pretrained_branch_next",
        "why": "AEROBLADE/FIRE-style reconstruction signals are a major SOTA direction and are not yet a full branch here.",
        "primary_gap": "reconstruction_lite now exposes deterministic resize-reconstruction residuals, but no pretrained autoencoder or diffusion-reconstruction detector branch is implemented yet.",
        "commands": "Run `python scripts/run_feature_baseline.py --feature-set reconstruction_lite ...` and `python scripts/train_physics_guided_net.py --physics-feature-set reconstruction_lite ...`, then add cached autoencoder/diffusion-reconstruction residual features.",
