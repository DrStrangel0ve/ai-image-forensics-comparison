...
TASKS = [
    ...
    },
    {
        ...
        "status": "runner_available_needs_benchmark",
        ...
        "primary_gap": "Neural/foundation branches still use global resized images in the best fused diagnostic.",
        "commands": "Run `python scripts/evaluate_tiled_frozen_encoder_model.py --model-template <frozen_run_template> --target-template <target_template> --output-dir <out>` for CLIP/DINOv2/ConvNeXt, then compare max/mean/top-k tile pooling against current global scores.",
        "exit_criterion": "Show whether tiled foundation branches improve reverse-transfer AUC beyond 0.8472 and reduce resize/blur stress losses.",
...
def build_closure_plan(sota_gap_path: Path) -> tuple[str, pd.DataFrame]:
    ...
    lines = [
        ...
        "2. In parallel, implement `high_res_tiled_foundation_eval`, because it directly targets the current resize/blur failure modes and can improve SCP-Fusion without waiting on external labels.",
