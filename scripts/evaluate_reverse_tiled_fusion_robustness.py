...
from evaluate_reverse_tiled_fusion import (
    load_tiled_target,
    summarize_by_score_mode,
)
from evaluate_reverse_tuned_fusion_robustness import (
    load_robust_target,
    resolved_robust_root,
    variant_slug,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate transformed-target reverse tuned fusion after replacing one robust "
            "target branch with native-tile scores."
        )
    )
    ...
    parser.add_argument(
        "--tile-detail",
        required=True,
        help="Per-image tiled score detail CSV for the transformed target variant.",
    )
    parser.add_argument("--tile-branch", default="dinov2_vits14")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "reports/ms_cocoai_to_ishu_tuned_fusion_{variant}_{tile_branch}_tiled_robustness_2026_06_14.md."
        ),
    )
...
def resolved_asset_prefix(args: argparse.Namespace) -> str:
    if args.asset_prefix:
        return str(args.asset_prefix)
    return (
        f"ms_cocoai_to_ishu_tuned_fusion_{variant_slug(args.variant)}_"
        f"tiled_{args.tile_branch}"
    )


def resolved_tiled_report_path(args: argparse.Namespace) -> Path:
    if args.report_path:
        return Path(args.report_path)
    return Path(
        f"reports/ms_cocoai_to_ishu_tuned_fusion_{variant_slug(args.variant)}_"
        f"tiled_{args.tile_branch}_2026_06_14.md"
    )
...
def run_tiled_robustness(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def write_report(
    ...
    rebuild_command = (
        ".\\.venv\\Scripts\\python.exe scripts\\evaluate_reverse_tiled_fusion_robustness.py "
        f"--variant {args.variant} "
        f"--tile-branch {args.tile_branch} "
        f"--tile-detail {args.tile_detail} "
        f"--asset-prefix {resolved_asset_prefix(args)} "
    ...
    lines = [
        f"# MS COCOAI to Ishu Tuned-Fusion {args.variant} Tiled-Branch Robustness",
        "",
        (
            "This evaluates the already-selected reverse tuned-fusion operating point on "
            f"`{args.variant}`-transformed Ishu target splits after replacing the transformed "
            f"`{args.tile_branch}` branch with native-tile aggregate scores. Source rows, "
            "fusion configuration, and threshold policy stay fixed."
        ),
        "",
        "## Summary",
        "",
        ...
        (
            f"The best transformed tiled-branch accuracy mode is `{best_accuracy['score_mode']}` "
            f"at {best_accuracy['target_accuracy_mean']:.4f}; the best ranking mode is "
...
def main() -> None:
    ...
    prefix = resolved_asset_prefix(args)
    detail_path = summary_dir / f"{prefix}_detail.csv"
    summary_path = summary_dir / f"{prefix}_summary.csv"
    detail.to_csv(detail_path, index=False)
    summary.to_csv(summary_path, index=False)
    clean_selected = pd.read_csv(args.selected_configs)
    report_path = resolved_tiled_report_path(args)
    write_report(args, summary, detail, clean_selected, report_path)
