...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="reports/assets/submission_table_reconstruction_ablation.csv",
        help="Compact reconstruction ablation table generated by build_submission_result_tables.py.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Paired repeated-seed support table generated by build_paired_seed_statistical_support.py.",
    )
...
def _reconstruction_delta(reconstruction_ablation: pd.DataFrame, setting: str) -> str:
    rows = reconstruction_ablation[
        (reconstruction_ablation["setting"] == setting)
        & (reconstruction_ablation["method"] == "reconstruction_v2")
    ]
    if rows.empty:
        raise ValueError(f"Missing reconstruction_v2 ablation row for setting={setting!r}")
    return f"{float(rows.iloc[0]['delta_auc_vs_reconstruction_lite']):+.4f}"


def _paired_rows_by_key(paired_seed_support: pd.DataFrame) -> dict[tuple[str, str], pd.Series]:
    required = {"comparison_id", "metric", "raw_delta_mean", "candidate_wins", "n_paired_seeds"}
    missing = required - set(paired_seed_support.columns)
    if missing:
        raise ValueError(f"Missing paired-seed support columns: {', '.join(sorted(missing))}")
    return {
        (str(row["comparison_id"]), str(row["metric"])): row
        for _, row in paired_seed_support.iterrows()
    }


def _paired_row(
    paired_rows: dict[tuple[str, str], pd.Series],
    comparison_id: str,
    metric: str,
) -> pd.Series:
    key = (comparison_id, metric)
    if key not in paired_rows:
        raise ValueError(f"Missing paired-seed support row for comparison_id={comparison_id!r}, metric={metric!r}")
    return paired_rows[key]
...
def _paired_delta(
    ...
    row = _paired_row(paired_rows, comparison_id, metric)
    return f"{float(row['raw_delta_mean']):+.4f}"
...
def _paired_wins(
    ...
    if pd.isna(wins):
        return "n/a"
    return f"{int(float(wins))}/{int(float(row['n_paired_seeds']))}"
...
def build_sections(
    ...
    reconstruction_ablation = pd.read_csv(reconstruction_ablation_path)
    ...
    paired_rows = _paired_rows_by_key(pd.read_csv(paired_seed_support))
    ...
    recon_same_delta = _reconstruction_delta(reconstruction_ablation, "ishu_same_bounded")
    recon_transfer_delta = _reconstruction_delta(reconstruction_ablation, "ishu_to_ms_cocoai_bounded")
    physics_acc_delta = _paired_delta(paired_rows, "ishu_physics_guided_vs_resnet18", "accuracy")
    physics_auc_delta = _paired_delta(paired_rows, "ishu_physics_guided_vs_resnet18", "AUC")
    physics_acc_wins = _paired_wins(paired_rows, "ishu_physics_guided_vs_resnet18", "accuracy")
    v4_auc_delta = _paired_delta(paired_rows, "combined_v4_ishu_to_ms_selectk60_vs_v3", "AUC")
    v4_auc_wins = _paired_wins(paired_rows, "combined_v4_ishu_to_ms_selectk60_vs_v3", "AUC")
    v4_ece_delta = _paired_delta(paired_rows, "combined_v4_ishu_to_ms_selectk60_vs_v3", "ECE")
    v4_ece_wins = _paired_wins(paired_rows, "combined_v4_ishu_to_ms_selectk60_vs_v3", "ECE")
    scp_clip_auc_delta = _paired_delta(paired_rows, "ishu_to_ms_scp_all_foundation_vs_clip", "AUC")
    scp_clip_auc_wins = _paired_wins(paired_rows, "ishu_to_ms_scp_all_foundation_vs_clip", "AUC")
    source_cal_brier_delta = _paired_delta(
        paired_rows,
        "ishu_to_ms_source_calibrated_all_foundation_vs_clip",
        "Brier",
    )
    source_cal_ece_delta = _paired_delta(
        paired_rows,
        "ishu_to_ms_source_calibrated_all_foundation_vs_clip",
        "ECE",
    )
    reverse_acc_delta = _paired_delta(paired_rows, "ms_to_ishu_temp_balanced_fusion_vs_clip", "accuracy")
    reverse_ece_delta = _paired_delta(paired_rows, "ms_to_ishu_temp_balanced_fusion_vs_clip", "ECE")
    ...
    sections = [
        _section(
            ...
            (
                ...
                f"to {_fmt(physics, 'accuracy')} accuracy and {_fmt(physics, 'auc')} AUC. The paired-seed support "
                f"table sharpens that anchor: physics-guided ResNet is up {physics_acc_delta} accuracy and "
                f"{physics_auc_delta} AUC versus vanilla ResNet with {physics_acc_wins} favorable accuracy seeds, "
                f"while Ishu-to-MS all-foundation fusion trails CLIP by {scp_clip_auc_delta} AUC. The transfer results "
        ...
        _section(
            ...
            (
                ...
                "audits, and artifact manifests are part of the anonymized supplementary artifact package so that "
                "results can be traced back to specific exports rather than opaque benchmark names. The paper should "
                "state that raw datasets and large model artifacts are external, while reports, prediction-derived "
                "tables, figures, manifests, and reproduction commands are bundled for review. The source-holdout "
                "stress table adds a generator-level "
        ...
        _section(
            ...
            (
                ...
                f"{_fmt(physics, 'accuracy')} accuracy and {_fmt(physics, 'auc')} AUC, with paired-seed support of "
                f"{physics_acc_delta} accuracy, {physics_auc_delta} AUC, and {physics_acc_wins} favorable accuracy "
                f"seeds versus vanilla ResNet. Cross-domain transfer changes "
                ...
                f"{_fmt(fusion, 'accuracy')} accuracy and {_fmt(fusion, 'auc')} AUC; the paired all-foundation "
                f"candidate trails by {scp_clip_auc_delta} AUC with {scp_clip_auc_wins} favorable AUC seeds, while "
                f"source-calibrated all-foundation improves Brier/ECE by {source_cal_brier_delta}/{source_cal_ece_delta}. "
                f"The triage result is more "
                ...
                f"{_fmt(reverse, 'auc')} AUC, with paired reverse support of {reverse_acc_delta} accuracy and "
                f"{reverse_ece_delta} ECE versus CLIP, and the bounded native-tiled conventional branch diagnostic improves "
                ...
                f"{source_stress['fake_miss_rate']} fake-miss rate. The reconstruction ablation is deliberately "
                f"caveated: `reconstruction_v2` changes AUC by {recon_same_delta} same-domain but "
                f"{recon_transfer_delta} under Ishu-to-MS transfer versus `reconstruction_lite`. A tiled-DINO follow-up gives "
                ...
                f"{tiled_dino['tile_mean_ece_count']} probes. Select-k60 `combined_v4` is therefore a guarded "
                f"follow-up: paired transfer AUC is {v4_auc_delta} with {v4_auc_wins} favorable seeds, while ECE is "
                f"{v4_ece_delta} with {v4_ece_wins} favorable seeds. "
        ...
        _section(
            ...
            (
                ...
                "source-threshold artifact. Paired-seed support should be presented as a robustness qualifier: "
                "physics-guided gains are stable on Ishu, while all-foundation transfer remains a calibration and "
                "diagnostic branch rather than a CLIP replacement. The DFF framing can carry the tiled-DINO mode tradeoff as a small "
                "robustness design rule, then connect it to the `combined_v4` and reconstruction roadmap. The "
                f"`reconstruction_v2` table is an ablation caveat rather than a new lead method because it moves "
                f"AUC by {recon_same_delta} on Ishu but {recon_transfer_delta} on transfer. The paper should "
        ...
        _section(
            ...
            (
                ...
                "different winners, so comparisons must name the objective. The anonymized supplementary artifact package "
                "contains generated result tables, LaTeX fragments, paper skeletons, lint reports, literature maps, "
                "draft BibTeX, and reproduction commands. The final paper still needs official venue templates, verified bibliography metadata, "
