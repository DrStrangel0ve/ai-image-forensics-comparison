...
def test_publication_control_suite_dry_run_lists_ordered_commands(tmp_path: Path) -> None:
    ...
    assert order["calibration operating modes"] < order["paired seed statistical support"]
    assert order["paired seed statistical support"] < order["claim matrix"]
    ...
    assert order["paired seed statistical support"] < order["method family comparison"]
