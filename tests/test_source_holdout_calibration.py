...
def test_source_holdout_calibration_keeps_seed_groups_separate(tmp_path: Path) -> None:
    ...
    out_dir = tmp_path / "source_holdout_calibration"
    ...
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "summarize_source_holdout_calibration.py"),
            "--metadata",
            str(metadata_path),
            "--predictions",
    ...
    detail = pd.read_csv(out_dir / "source_holdout_calibration.csv")
    summary = pd.read_csv(out_dir / "source_holdout_calibration_summary.csv")
